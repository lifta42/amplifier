...
DFAuto dfa_create(void)
{
  ...
  assert(dfa != NULL);
  dfa->capacity = 16;
  dfa->states   = malloc(sizeof_field(struct _DFAuto, states) * dfa->capacity);
...
void dfa_connect(DFAuto dfa, const DFAState from, const DFASymbol with,
                 const DFAState to)
{
  int state_length = dfa->states[from].length;
  extend_when_necessary(dfa->states[from].records, state_length,
                        &dfa->states[from].capacity,
